...
function service(request = () => { throw new Error("Unexpected request"); }) {
  const source = readFileSync(new URL("../src/services/home.ts", import.meta.url), "utf8");
...
test("card and package expiry distinguish yesterday, today, future and missing dates", () => {
  ...
  assert.equal(cardStatus({ IsActivated: true, EndDate: "2026-09-16" }, now), "Expired");
  assert.equal(cardStatus({ IsActivated: false, EndDate: "2026-09-18" }, now), "Inactive");
  assert.equal(cardStatus({ IsActivated: true, EndDate: "2026-09-18" }, now), "Active");
});
...
test("loads every applicable dashboard endpoint once and uses fresh profile for group and KYC", async () => {
  ...
  const { loadHomeData } = service(async (path, options) => {
    ...
    if (path.includes("Customer/GetById")) return [{ MemberId: 7, GroupId: 9, AadhaarNumber: "test", FaceIdentityImage: "photo" }];
    if (path.includes("Group/GetById")) return [{ GroupName: "Community" }];
    if (path.includes("GetMemberProducts")) return [{ Products: [{ ProductName: "Older", IssuedOn: "2025-01-01" }, { ProductName: "Latest", IssuedOn: "2026-01-01" }] }];
    if (path.includes("GetMemberCard")) return { status: true, returnData: [{ IsActivated: true }] };
    if (path.includes("ConsultationList")) return [];
    if (path === "ConfigValues/all") return [{ ConfigKey: "HealthTip", ConfigValue: "Daily tip" }];
    if (path === "Products/all") return [{ ProductName: "Free", IsFree: true, IsActive: true }, { ProductName: "Inactive", IsFree: true, IsActive: false }];
    return { status: true };
  });
  ...
  assert.equal(calls.find(call => call.path.endsWith("KYCVerifiedOrNot")).body.aadhaarNumber, "test");
...
test("partial failures retain successful data and distinguish errors from empty records", async () => {
  ...
  assert.ok(result.errors.includes("Appointments unavailable. Please try again later."));
});
...
test("latest active package skips expired, inactive and future packages without mutating the list", () => {
  ...
  const products = [
    { ProductName: "Older active", IssuedOn: "2026-01-01", ValidTill: "2026-12-31" },
    { ProductName: "Expired", IssuedOn: "2026-09-16", ValidTill: "2026-09-16", IsActive: true },
    { ProductName: "Inactive", IssuedOn: "2026-09-15", ValidTill: "2026-12-31", IsActive: false },
    { ProductName: "Latest active", IssuedOn: "2026-09-01", ValidTill: "2026-09-17" },
  ...
  assert.equal(latestActivePackage([{ ProductName: "Unknown" }], now), undefined);
});
...
test("community customers use fallback without sending invalid member requests", async () => {
  ...
  const { loadHomeData } = service(async path => {
    paths.push(path);
    return path.includes("CommunityCustomers") ? [{ Name: "Community customer" }] : [];
  });
...
test("upcoming appointments exclude past and cancelled bookings and sort earliest first", () => {
  ...
  const appointment = (id, date, status = "Booked") => ({ BookingConsultationId: id, AppointmentDate: date, StatusName: status, IsCouponClaimed: true });
  assert.equal(nextAppointment([
    appointment(1, "2026-09-16"), appointment(2, "2026-09-18"),
    appointment(3, "2026-09-17", "Cancelled"), appointment(4, "2026-09-17"),
  ], new Date(2026, 8, 17, 12)).BookingConsultationId, 4);
});
