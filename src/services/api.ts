...
export async function apiRequest<T>(
  ...
  const base = (path.startsWith("lambdaAPI/")
    ? import.meta.env.VITE_API_BASE_URL
    : import.meta.env.VITE_LEGACY_API_BASE_URL)?.replace(/\/$/, "");
  ...
  if (!response.ok) throw new Error(`Customer service request failed (${response.status}).`);
