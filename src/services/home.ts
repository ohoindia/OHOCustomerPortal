...
export const fetchMemberProducts = (id: number, signal: AbortSignal) =>
  apiRequest<Array<{ Products?: MemberProduct[] }>>(
    `lambdaAPI/Customer/GetMemberProducts/${id}`,
    { signal },
  );
...
export const fetchConfigValues = (signal: AbortSignal) =>
  apiRequest<ConfigEntry[]>("ConfigValues/all", {
    body: { skip: 0, take: 0 },
    signal,
  });
export const fetchProducts = (signal: AbortSignal) =>
  apiRequest<MemberProduct[]>("Products/all", {
    body: { skip: 0, take: 0 },
    signal,
  });
...
export async function loadHomeData(
  ...
  const [
    ...
    hasMember
      ...
      : null,
    hasMember
      ? read("Appointments", fetchAppointments(memberId, signal), Array.isArray)
      : null,
    hasMember
      ? read(
      ...
      : null,
    hasMember
      ? read("PAN verification", fetchPANStatus(memberId, signal), verification)
      : null,
    read(
